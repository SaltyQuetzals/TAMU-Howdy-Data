...
const PROF_CACHE = new Map<string, CompassFaculty>();
...
export class Term {
  ...
  async courses(dept: string): Promise<CompassCourse[]> {
    ...
    const compassCourses = await this.fetchAndCombinePaginatedResponses<
      CompassCourse
    >(endpoint, urlString =>
      this.client
        .get(urlString)
  ...
  async faculty(
    ...
    for (const facultyMember of facultyMembers) {
      if (!PROF_CACHE.has(facultyMember.displayName)) {
        ...
        PROF_CACHE.set(facultyMember.displayName, fullMember);
      }
      fullFaculty.push(PROF_CACHE.get(facultyMember.displayName)!);
    }
    return fullFaculty;
  }

  static async allTerms(): Promise<
